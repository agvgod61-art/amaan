...
export default function Checkout() {
  ...
  const handleQuickWhatsapp = () => {
    ...
    const waMessage = `🏁 *QUICK ORDER ENQUIRY* 🏁\n\n` +
      `🛍️ *I want to order:*\n${orderItemsText}\n\n` +
      `💵 *Total Cart Value:* ₹${totalPrice.toLocaleString('en-IN')}\n\n` +
      `_Please help me process this order._`;
    ...
    return (
      <div className="max-w-xl mx-auto py-24 px-6 text-center">
        ...
        <div className="bg-white/5 border border-white/10 p-6 mb-8 text-left">
          ...
          <a 
            href={whatsappUrl || `https://wa.me/918292908076?text=${encodeURIComponent(`Hi, I just placed an order with ID: ${orderId}. Please verify my delivery details. Name: ${customerInfo.name}`)}`}
            target="_blank"
                ...
                <div id="delivery-form" className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2 mt-8 flex flex-col sm:flex-row gap-4">
                    <button 
                      ...
                      className={cn(
                        "bg-brand-accent text-white px-12 py-5 text-xs font-bold uppercase tracking-[0.2em] transition-all flex-grow shadow-[0_10px_30px_rgba(226,43,43,0.3)] group flex items-center justify-center gap-3",
                        isSubmitting ? "opacity-50 cursor-not-allowed" : "hover:bg-red-700"
                      )}
                    >
                      <span>{isSubmitting ? "Processing..." : "Place Order & Pay via WhatsApp"}</span>
          ...
          <div className="bg-white/5 border border-white/10 p-8 sticky top-28">
            ...
            <div className="mt-6">
              <button
                ...
                className="w-full bg-[#25D366] text-[#075e54] py-4 text-xs font-bold uppercase tracking-widest hover:bg-[#20bd5a] transition-all flex items-center justify-center gap-3 shadow-[0_4px_20px_rgba(37,211,102,0.2)]"
              >
